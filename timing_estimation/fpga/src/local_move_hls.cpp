...
#ifndef F
#error "F is not defined. Compile with -F=<bits> where F = ceil(3.5 * log2(SK_N / EPS_DISCR))."
#endif
...
#pragma HLS UNROLL
    ...
    // Extract the selected spin s_i from the same one-hot flip mask.
    delta_t si = selected_spin(spins, flip_mask);

    // Final normalized local energy difference: 2 s_i field_i / alpha.
    return delta_t(delta_t(2) * si * field_over_alpha);
}
