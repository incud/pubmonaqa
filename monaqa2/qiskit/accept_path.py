...
class AcceptPath(Gate):
    ...
    def __init__(self, n: int, coins: int = 3, mocked_circuit: bool = False, label=None) -> None:
        if n < 2:
        ...
        super().__init__("AcceptPath", 2 * self.n + self.coins + self.n_work, [], label=label)
    ...
    def layout(self) -> dict[str, list[int]]:
        ...
        if self.mocked_circuit:
            return {"A": a, "B": b, "coins": coins, "flag": [], "fanout": [], "work": []}

        flag = 2 * self.n + self.coins
        fanout = list(range(flag + 1, self.num_qubits))
        return {"A": a, "B": b, "coins": coins, "flag": [flag], "fanout": fanout, "work": [flag] + fanout}
    ...
    def _toggle_zero_flag(self, qc: QuantumCircuit, coins: list[int], flag: int, clean: list[int]) -> None:
        """
        ...
        Larger cases use Qiskit's two-clean-ancilla KG24 MCX synthesis.
        """
        if len(coins) == 0:
    ...
    def _build_mocked_definition(self) -> QuantumCircuit:
        ...
        zero_controlled_swap = SwapGate().control(num_ctrl_qubits=len(coins), ctrl_state=0)
