...
def _positive_y_values_from_primary_axes(axes: Sequence[plt.Axes]) -> np.ndarray:
    values = []
    for ax in axes:
...
def _log_limits_and_ticks_for_queries(
    row_axes: Sequence[plt.Axes],
    tick_order_step: int,
) -> tuple[float, float, np.ndarray]:
    if tick_order_step <= 0:
    ...
    y = _positive_y_values_from_primary_axes(row_axes)
    if y.size == 0:
        return 1.0, 1e3, np.array([1.0, 1e3], dtype=float)

    ymin = float(np.min(y))
    ymax = float(np.max(y))
    min_order = int(np.floor(np.log10(ymin)))
    max_order = int(np.ceil(np.log10(ymax)))

    lower_order = min(0, int(tick_order_step * np.floor(min_order / tick_order_step)))
    upper_order = max(0, int(tick_order_step * np.ceil(max_order / tick_order_step)))

    if lower_order == upper_order:
        upper_order += tick_order_step

    lower = 10.0 ** lower_order
    upper = 10.0 ** upper_order
    ticks = 10.0 ** np.arange(lower_order, upper_order + 1, tick_order_step, dtype=float)
    return lower, upper, ticks
...
def _apply_rowwise_last_step_y_limits(
    ...
    for row in range(nrows):
        ...
        row_end = min((row + 1) * ncols, n_items)
        row_axes = list(axes[row_start:row_end])
        lower, upper, ticks = _log_limits_and_ticks_for_queries(row_axes, y_tick_order_step)
