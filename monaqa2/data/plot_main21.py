...
def plot_annealing_classical_and_quantum_runtime_vs_n(
    ...
    def _plot_runtime_line(values: list[float] | np.ndarray, color: str, label: str, zorder: int = 6):
        ...
        (handle,) = ax.plot(n_vals, values, color=color, linewidth=1.35, linestyle="-", alpha=0.95, zorder=zorder, label=label)
        return handle
    ...
    for proposal in proposals_to_plot:
        ...
        classical_label = _walk_label(proposal, "classical")
        quantum_label = _walk_label(proposal, "quantum")
        ...
        if proposal == "uniform":
            ...
            classical_handle = _fill_area_with_projection(classical_min, classical_max, color=color_classical, label=classical_label, alpha=0.30, zorder=2, edge_linewidth=classical_edge)
            handles.append(classical_handle)
            labels.append(classical_label)

        quantum_handle = _fill_area_with_projection(quantum_min, quantum_max, color=color_quantum, label=quantum_label, alpha=0.34, zorder=3, edge_linewidth=area_edge_linewidth)
