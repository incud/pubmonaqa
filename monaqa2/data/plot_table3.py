...
def _replace_legend_with_line_fit_rows(ax: plt.Axes, ncol: int = 3, y: float = -0.16, row_gap: float = 0.045, group_gap: float = 0.16, line_half_width: float = 0.060, fontsize: int | None = None) -> None:
    ...
    for group_start in range(0, len(handles), ncol):
        ...
        xs = np.linspace(0.16, 0.84, count) if count > 1 else np.array([0.5])
...
def plot_spectral_gap_vs_beta_table(
    ...
    **kwargs,
    ...
    for fixed_n, ax in zip(fixed_ns, axes):
        plot_spectral_gap_vs_beta(fixed_n=fixed_n, fig=fig, ax=ax, title=rf"$n={fixed_n}$", **kwargs)
        _replace_legend_with_line_fit_rows(ax, ncol=3, y=legend_y)
