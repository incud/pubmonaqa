...
def _runtime_from_queries(n: int, walk: str, move: str, num_queries: float, num_trotter_steps: int) -> float:
    ...
    if walk == "quantum" and move in ["qemc", "layden"]:
        return float(get_time_quantum_walk_qemc(n, q, num_trotter_steps))
...
def plot_classical_logicalquantum_runtime_vs_n(
    ...
    The query table contains observed points and fitted query coefficients A_q,b. This function converts observed num_queries to runtime, fixes the exponential term exp(b n) from the query fit, fits only the runtime prefactor C in C P_move(n) exp(b n), and draws the fitted line on n=3,...,100.
    """
