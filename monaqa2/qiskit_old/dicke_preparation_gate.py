...
def cry_via_cx(qc: QuantumCircuit, theta: float, control: int, target: int) -> None:
...
def ccry_via_ccx(qc, theta, c0, c1, target):
...
class SCS(Gate):
    ...
    def _gate_i(qc: QuantumCircuit, n: int, wires: list[int]) -> None:
        ...
        # CRY = RY(theta/2) CX RY(-theta/2) CX, RY = S H RZ H S^dag. Thus uses only 2 Rz
        cry_via_cx(qc, theta, wires[1], wires[0])
        qc.cx(wires[0], wires[1])
    ...
    def _gate_ii_l(qc: QuantumCircuit, l: int, n: int, wires: list[int]) -> None:
        ...
        # Replaced by 2 toffolis and 2 Ry
        # qc.mcry(theta, [wires[2], wires[1]], wires[0], None, mode="noancilla")
        ccry_via_ccx(qc, theta, wires[2], wires[1], wires[0])
        qc.cx(wires[0], wires[2])
    ...
    def _controlled_addition(
        ...
        for ell in range(len(reg_a) - 1, -1, -1):
            ...
            for j in range(min(len(reg_b), ell + 1)):
                ...
                if len(ctrls) == 1:
                    # 2 x CNOT + 2 x RY
                    cry_via_cx(qc, theta, ctrls[0], reg_b[j])
                elif len(ctrls) == 2:
                    # 2 x Toffoli + 2 x RY
                    # qc.mcry(theta, ctrls, reg_b[j], None, mode="noancilla")
                    ccry_via_ccx(qc, theta, ctrls[0], ctrls[1], reg_b[j])
                else:
